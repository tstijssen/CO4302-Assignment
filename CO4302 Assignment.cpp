...
const EKeyCode kSpeedUpKey = Key_Numpad1;
const EKeyCode kSpeedDownKey = Key_Numpad0;
const EKeyCode kCameraForward = Key_E;
const EKeyCode kCameraBackward = Key_Q;
...
const EKeyCode kDisableText = Key_1;
const EKeyCode kToggleDeaths = Key_2;
const EKeyCode kToggleTiming = Key_3;
const EKeyCode kTogglePartitions = Key_4;

// testing
const EKeyCode kMoveForward = Key_I;
const EKeyCode kMoveBack = Key_K;
const EKeyCode kMoveLeft = Key_J;
const EKeyCode kMoveRight = Key_L;
const EKeyCode kMoveUp = Key_O;
const EKeyCode kMoveDown = Key_U;
const EKeyCode kActivatePlayer = Key_Return;
...
void main()
{
	...
	sBallCollisionData collisionData[kNumOfSpheres];
	...
	while (myEngine->IsRunning())
	{
		...
		/*if (playing)
		{
			...
			{
				collisionData[kNumOfStatics].m_Velocity.z += kPlayerSpeed;
			}

			if (myEngine->KeyHeld(kMoveBack))
			{
				collisionData[kNumOfStatics].m_Velocity.z -= kPlayerSpeed;

			}

			if (myEngine->KeyHeld(kMoveLeft))
			{
				collisionData[kNumOfStatics].m_Velocity.x -= kPlayerSpeed;

			}

			if (myEngine->KeyHeld(kMoveRight))
			{
				collisionData[kNumOfStatics].m_Velocity.x += kPlayerSpeed;

			}

			if (myEngine->KeyHeld(kMoveUp))
			{
				collisionData[kNumOfStatics].m_Velocity.y += kPlayerSpeed;
			}

			if (myEngine->KeyHeld(kMoveDown))
			{
				collisionData[kNumOfStatics].m_Velocity.y -= kPlayerSpeed;
			}
		}*/
		...
		if (textEnabled)
		{
			myFont->Draw("FrameTime = " + std::to_string(frameTime) + "\n" +
				...
				"Sphere Speed = " + std::to_string(speedMod), 0, 0, textColour);

			if (sphereDeath)
			{
				myFont->Draw("SphereDeath = TRUE", 0, 200, kRed);
			}
			if (g_DETAILED_TIMING)
			{
				myFont->Draw("Detailed Timing = TRUE", 0, 220, kRed);
			}
		}
		...
		//if (myEngine->KeyHit(kActivatePlayer) && !playing)
		//{
		//	playing = true;
		//	myCamera->AttachToParent(spheres[kNumOfStatics]);
		//	myCamera->SetLocalPosition(0, 20, -100);
		//	spheres[kNumOfStatics]->SetSkin(whiteBallSkin);
		//	collisionData[kNumOfStatics].m_Velocity = { 0,0,0 };
		//	collisionData[kNumOfStatics].m_Position = { 0,0,0 };
		//	modelData[kNumOfStatics].m_Health = 100;
		//	modelData[kNumOfStatics].m_Name = "PLAYER";
		//}
